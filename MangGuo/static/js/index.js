	...
	$(window).scroll(function(){
		if($(window).width() >= 992){
			...
			if($(window).scrollTop() >= 520){
				$(".cttitle>h1,.cttitle>p").addClass("endstatus");
			}
			if($(window).scrollTop() >= 560){
				$(".ctmain>div:eq(0)").addClass("endstatus");
			}
			if($(window).scrollTop() >= 1100){
				$(".ctmain>div:eq(1)").addClass("endstatus");
			}
			if($(window).scrollTop() >= 1500){
				$(".ctmain>div:eq(2)").addClass("endstatus");
			}
			if($(window).scrollTop() >= 2000){
				$(".ctmain+div.more,.news>div:first>h2").addClass("endstatus");
			}
			if($(window).scrollTop() >= 2100){
				$(".news .row>div:lt(2)").addClass("endstatus");
			}
			if($(window).scrollTop() >= 2250){
				$(".news .row>div:lt(4)").addClass("endstatus");
			}
			if($(window).scrollTop() >= 2400){
				$(".news .row>div").addClass("endstatus");
			}
		}else{
			$(".navbar-brand").show();
			$("nav.navbar").css("top","0px");
