	...
	var router=express.Router();
     router.get('/',(req,res)=>{
       switch(req.query.act){//act有几种 所以考虑使用switch
            case 'mod'://修改
              db.query(`SELECT * FROM blog_table WHERE id=${req.query.id}`,(err,data)=>{
                if(err){
                    console.log();
                  ...
                  db.query(`SELECT * FROM blog_table`,(err,blogs)=>{
                   ...
                   res.status(500).send('database error').end();
                       ...
                       res.render('admin/blog.ejs',{blogs,mod_data: data[0]});
                    }
                   });
                 }
              });
            ...
            db.query(`DELETE FROM blog_table WHERE ID=${req.query.id}`,(err,data)=>{
             if(err){
              ...
              res.status(500).send('database error').end();      
                ...
                res.redirect('/admin/blog');//数据删除依然重新redirect
             }
            });
            break;
            default://默认正常渲染
            ...
            if(err){
             ...
             // console.log(blogs);
             res.render('admin/blog.ejs',{blogs:data})
            }
       });
       }  
     });
     router.post('/',(req,res)=>{
        ...
        if(!username || !title || !content || !time || !view || !imgUrl){
            res.status(400).send('arg error').end();
             ...
             db.query(`INSERT INTO blog_table (username,title,content,time,view,imgUrl) VALUE('${username}','${title}','${content}','${time}','${view}','${imgUrl}')`,(err,data)=>{
             if(err){
               res.status(500).send('database error').end();
             }else{
				// res.send('ok了').end();
